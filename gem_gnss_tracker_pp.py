...
class PurePursuit(object):
    ...
    def __init__(self):
        ...
        self.desired_speed = 0.8 # m/s, reference speed
        ...
        # Steering angle request controller (TO DO)
        self.steer_rqst_sub = rospy.Subscriber('/pacmod/as_rx/steering_rqst', PositionWithSpeed, self.steering_rqst_callback)
        ...
        # Acceleration request controller (TO DO)
        self.accel_rqst_pub = rospy.Subscriber('/pacmod/as_rx/accel_rqst', PacmodCmd, self.accel_rqst_callback)
    ...
    # Steering angle request callback function (TO DO)
    def steering_rqst_callback(self, msg):
        self.steer_rqst.angular_position = round(msg.angular_position, 2)
        self.steer_rqst.angular_velocity_limit = round(msg.angular_velocity_limit, 1)

    def accel_rqst_callback(self, msg):
        self.accel_rqst.f64_cmd = round(msg.f64_cmd, 1)
    ...
    def start_pp(self):
        ...
        while not rospy.is_shutdown():
            ...
            output_brake = 0.0
            if self.accel_rqst.f64_cmd > 8:
                output_accel = self.pid_speed.get_control(current_time, self.desired_speed - filt_vel)
                output_brake = 0.0
                print("No person detected")
            elif self.accel_rqst.f64_cmd > 4:
                output_accel = 0.0
                output_brake = 0.1
                print("Person detected far, slowing down")
            else:
                output_accel = 0.0
                output_brake = 0.75
            ...
            self.accel_pub.publish(self.accel_cmd)
            self.brake_pub.publish(self.brake_cmd)
            self.steer_pub.publish(self.steer_rqst)            # self.turn_pub.publish(self.turn_cmd)
